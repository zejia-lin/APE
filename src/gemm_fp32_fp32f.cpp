...
void gemm_fp32_fp32f(APEHandler apeHandle, ApeTrans transa, ApeTrans transb, int m, int n, int k, const float *alpha, const float *A, int lda,
    ...
    assert((1ULL * m * k + k * n) * 4 <= apeHandle.getBufSize());
    half *buf = (half *)apeHandle.getBuf();
    half *buf_a, *buf_b;
    buf_a = (half *)apeHandle.getBuf();
    buf_b = (half *)(apeHandle.getBuf()) + m * k * 2;

    split_fp32_to_fp16(buf_a, A, m * k);
    split_fp32_to_fp16(buf_b, B, k * n);

    float alpha0 = *alpha, alpha1 = *alpha / 4096.0f, beta0 = *beta, beta1 = 1;
    cublasSafeCall(cublasGemmEx(apeHandle.getCublasHandle(), cublasOperation_t(transa), cublasOperation_t(transb), m, n, k,
